...
import groupDayData from '../../api/mock/groupDay.json'; // 목업 데이터
import { useNavigate } from 'react-router-dom';
import { addRoutine } from '../../redux/routine'; // Add this import
...
const Main = () => {
  ...
  const routines = useSelector((state) => state.routine.routines); // Get routines from the store
  const dispatch = useDispatch();
  ...
  useEffect(() => {
    setCalendarData({
      personal: routines || [], // Fetch routines from the store
      groups: groupDayData.groups || [], // 안전하게 빈 배열로 초기화
    });
    dispatch(setSelectedSection('view1'));
  }, [dispatch, routines]);
